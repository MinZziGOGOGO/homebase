...
CALENDAR_FILE = REAL_HOME / ".hermes" / "calendar.json"
...
def _parse_iso(dt_str: str) -> datetime:
    """Parse ISO datetime, with or without timezone."""
    dt_str = dt_str.replace("Z", "+00:00")
    if dt_str.endswith("+00:00") and "+" not in dt_str[:-6]:
...
async def list_calendar_events(start: str | None = None, end: str | None = None):
    """List calendar events, optionally filtered by date range."""
    data = _load_calendar()
    events = data.get("events", [])
    if start or end:
        range_start = _parse_iso(start) if start else datetime.min
        range_end = _parse_iso(end) if end else datetime.max
        filtered = []
        for ev in events:
            ev_start = _parse_iso(ev["start"])
            ev_end = _parse_iso(ev["end"]) if ev.get("end") else ev_start
...
async def delete_calendar_event(event_id: str):
    ...
    data["events"] = [ev for ev in data["events"] if ev.get("id") != event_id]
...
async def get_calendar_month(year: int | None = None, month: int | None = None):
    """Return events grouped by day for a given month.
